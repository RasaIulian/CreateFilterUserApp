...
const displayUsers = () => {
  ...
  users.forEach((user) => {
    ...
    if (usersList) {
      usersList.innerHTML += `<li data-id="${
        user.id
      }" onClick="handleUserClick(${user.id})">
          <b>ID</b>: ${user.id};
          <b>Name:</b> ${user.name}
          <b title="clicks counter"><span id="counter${user.id}" ${
...
const search = () => {
  ...
  if (usersList) {
    ...
    const isUserListEmptyOrMessage =
      usersList.innerHTML === "" || usersList.innerHTML === Message;
